...
| 3 | Architecture proposal is based on general knowledge, not verified source | Medium — could misdirect later phases if wrong | Phase 2 upstream analysis explicitly re-verifies every assumption listed in "Open items" of `docs/phase1-architecture-proposal.md` before any config/auth/transport code is written. |
...
| 5 | Removing rendezvous also removes whatever abuse-prevention it provided | Medium/High (security) | Treat brute-force lockout / rate limiting on the direct-IP listener as a hard requirement of the Authentication phase, not optional hardening. |
...
3. **Repository setup:** approved — proceed with `git init` + clone of upstream RustDesk per `docs/phase1-repository-plan.md`.
